...
two components of the algorithm: Indexing (preprocessing) and retrieval (query processing or search).
...
{% katexmm %} $\lambda$ {% endkatexmm %} vector with the largest {% katexmm %} $i$-th {% endkatexmm %}
...
relative to the {% katexmm %} $\lVert u \rVert_1$ {% endkatexmm %}, but also relative to other
...
inverted lists, it can therefore end up in multiple non-overlapping clusters.
...
{% katexmm %} $\langle q, s$ {% endkatexmm %} if {% katexmm %} $s$ {% endkatexmm %}
